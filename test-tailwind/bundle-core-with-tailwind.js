...
function F(e2, c, l = {}, n = "rebuild") {
  let g = new RegExp(`^<${c}[^>]*>(.*)</${c}>$`, "s");
  if (!g.test(e2.trim())) return B(c, l, e2);
  switch (n) {
    case "respect":
      return e2;
    case "preserve-attrs":
      ...
      return Object.entries(l).forEach(([s, i]) => {
        ...
        p.test(o) ? s === "class" ? o = o.replace(p, (d, u) => `class="${`${u} ${i}`.trim()}"`) : o = o.replace(p, `${s}="${i}"`) : o = o.replace(new RegExp(`<${c}([^>]*)>`, "i"), `<${c}$1 ${s}="${i}">`);
      }), o;
    case "rebuild":
    default:
      let t = e2.match(g), a = t ? t[1] : e2;
      return B(c, l, a);
  }
}
function w(e2, c) {
...
// dist/chunk-X6ZEOKPR.mjs
var R = { transform(t, r) {
  let n = T(t, this, r), e2 = "";
  if (t.innerHTML ? e2 = t.innerHTML : t.innerContent && t.innerContent.length > 0 && (e2 = t.innerContent.join("")), e2.trim() === "" && t.attrs?.content && (e2 = t.attrs.content), r.contentHandling) switch (r.contentHandling) {
    case "rendered":
      return e2;
    case "hybrid":
      if (r.cssFramework && r.cssFramework !== "none") return w(e2, r);
      break;
  }
  let s = r.renderedContentHandling || "rebuild";
  return F(e2, "p", { class: n }, s);
}, cssMapping: { tailwind: { block: "my-4 px-0", align: { left: "text-left", center: "text-center", right: "text-right" }, dropCap: "first-letter:float-left first-letter:text-7xl first-letter:font-bold first-letter:mr-3 first-letter:mt-1" }, bootstrap: { block: "mb-4 px-0", align: { left: "text-start", center: "text-center", right: "text-end" }, dropCap: "first-letter:float-start first-letter:fs-1 first-letter:fw-bold first-letter:me-2 first-letter:mt-1" } } };

// test-tailwind/test-core-with-tailwind.js
S("core/paragraph", R);
var blocks = [
  {
