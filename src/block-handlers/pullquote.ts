...
export const pullquoteBlockHandler: BlockHandler = {
  ...
  transform(block: Block, options: ConversionOptions): string | unknown {
    ...
    if (content.trim().startsWith('<blockquote') && content.trim().endsWith('</blockquote>')) {
      ...
      if (existingClassMatch) {
        ...
        content = content.replace(/^<blockquote/, `<blockquote class="${combinedClasses}"`);
      }
      ...
      if (style) {
        ...
        if (styleMatch) {
          content = content.replace(/style="([^"]*)"/, `style="${styleMatch[1]}; ${style}"`);
        } else {
          content = content.replace(/^<blockquote([^>]*)/, `<blockquote$1 style="${style}"`);
        }
      }
  ...
  cssMapping: {
    ...
    tailwind: {
      block:
        'text-xl italic font-medium border-l-0 border-r-0 border-t-4 border-b-4 border-gray-300 py-6 my-8 text-center',
      align: {
        left: 'text-left',
        center: 'text-center',
        right: 'text-right',
