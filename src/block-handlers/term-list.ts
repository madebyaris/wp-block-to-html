...
function generateTermPlaceholders(
  ...
  const termsByTaxonomy: Record<string, Array<{id: number, name: string, count: number, parent: number}>> = {
    'category': [
  ...
  const filteredTerms = showOnlyTopLevel 
    ? terms.filter(term => term.parent === 0)
    : terms;
